...
for spot in taipeiAttractions['data']['results']:
    spotTitle = spot['stitle']
    for mrt in stationMRT['data']:
        if mrt['SERIAL_NO'] == spot['SERIAL_NO']:
            spotDistrict = mrt['address'][5:8]
    spotLongitude = spot['longitude']
    spotLatitude = spot['latitude']
    spotImgURL = 'https' + spot['filelist'].split("https", 2)[1]
    tableSpot.append(
        [spotTitle, spotDistrict, spotLongitude, spotLatitude, spotImgURL])
